...
public class MemberController {
	...
	public String login(@RequestParam String m_userid, @RequestParam String m_password,Model model,HttpSession httpSession) {
		...
		// 아이디와 비번을 service에 보내어 DB를 조회하여
...
//		if(bUser) model.addAttribute("USERID",m_userid);
		...
		* 그로인해 session에 담긴 값ㅇ리 null이 될 것이다.
		* 
		...
		*/
		 ...
		 * server는 요청에 맞는 결과를 response(응답)
		 * 하게되고 client와 server의 연결은 즉시 중단된다.
		 * 이러한 프로토콜을 stateless(상태를 보관하지 않는)라고 한다.
		 * 
		 * 2.stateless환경에서는 client가 server와 연결할때 마다
		 ...
		 * 			처리하지 않을것인가를 판단
		 * 4. 초기에는 사용자의 로그인 정보를 web브랄우저에 cookie라는 형태로
		 * 		정보를 저장하고, 로그인 관련 일들을 수행했다.
		 * 		하지만 cookie는 불법적인 사용자에 의해 탈취될 수 있고,
		 * 		그로인해 보안상 문제가 발생할 수 있다.
		 * 5. 지금 사용자가 login을 정상적으로 수행하면 서버에 session이라는
		 * 		특별한 정보를 메모리에 생성하고 그 session의 id(key)를 
		 * 		response 정보에 실어서 client에게 보내주고
		 * 		이후 client는 request 정보에 이 session id를 같이 포함하여
		 * 		서버로 요청을 보낸다.
		 * 
		 * 6. 그러면 서버는 request 정보에 session id가 있는가를 확인하고
		 * 		그 id정보를 바탕으로 login된 정보를 취급하게 된다.
		 * 7. spring framework(java포함) 에서는 HttpSession 클래스를 제공해주어
