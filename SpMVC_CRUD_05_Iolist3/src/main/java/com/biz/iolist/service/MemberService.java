...
import com.biz.iolist.medel.MemberVO;
...
public class MemberService {
	...
	public int insert(MemberVO memberVO) {
		String strPassword = memberVO.getM_password();// plan text : 평문
		String cryptPassword = passwordEncoder.encode(strPassword); // crypt text : 암호문
		log.debug("비밀번호:" + strPassword + ":" + cryptPassword);

		memberVO.setM_password(cryptPassword);

		/*
		 * 최초로 가입되는 회원에게 ADMIN 권한을 부여하고 이후 부터는 USER권한을 부여한다.
		 */
