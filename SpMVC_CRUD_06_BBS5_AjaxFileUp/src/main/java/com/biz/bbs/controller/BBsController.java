...
//		LocalTime lt = LocalTime.now();
	...
	public String write_up(@ModelAttribute BBsReqDto bbsReqDto,
							Model model,
		 ...
		 * vo(Dto)에 한번 담긴 값은 웹브라우저가 열려있는 동안에
		 * 항상 유지되는 성질이 있다.
		 * 그래서, write(POST)를, update(POST)를 수행한 후에는
		 * SessionStatus의 setComplete() 메서드를 호출하여
		 * vo(Dto)에 담긴 값을 비워주어야한다. 
		 */
		sStatus.setComplete();
	...
	public String update(@RequestParam("bbsReqDto") long bbs_seq, Model model,HttpSession httpSession) {
	...
	public String update(@ModelAttribute("bbsReqDto") BBsReqDto bbsReqDto,Model model, SessionStatus sStatus) {
	...
	public String reply(
		...
		bbsReqDto.setBbs_date(curDate);
		bbsReqDto.setBbs_time(curTime);
		// 답글달기에서는 bbs_main_seq에 원글의 bbs_seq값을 포함해야 한다.
